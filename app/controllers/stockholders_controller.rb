class StockholdersController < ApplicationController
  ...
  def new
    @stockholder = current_user.stockholders.build
  end
  ...
  def create
    @stockholder = current_user.stockholders.build(stockholder_params)
  ...
  private
    ...
    def stockholder_params
      params.require(:stockholder).permit(:name, :email, :user_id)
    end
end
